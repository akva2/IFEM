...
protected:
  ...
  Mat A; //!< The actual ISTL matrix
  typedef Dune::MatrixAdapter<Mat,Vec,Vec> Operator;
  Operator op; //!< The matrix adapter
  std::unique_ptr<Dune::InverseOperator<Vec, Vec>> solver; //!< Solver to use
  std::unique_ptr<Dune::Preconditioner<Vec,Vec>> pre; //!< Preconditioner to use
  const ProcessAdm&   adm;             //!< Process administrator
