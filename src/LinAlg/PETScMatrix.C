    ...
    for (size_t j = 0; j < cols(); ++j) {
      for (int i = IA[j]; i < IA[j+1]; ++i) {
        ...
        for (size_t b = 0; b < blocks && (iblk == -1 || jblk == -1); ++b) {
          ...
          if (iblk == -1 && (it = dd.getBlockEqs(b).find(JA[i]+1)) != dd.getBlockEqs(b).end()) {
            iblk = b;
            eq2b[JA[i]][0] = b;
            eq2b[JA[i]][1] = glb2Blk[i][1] = std::distance(dd.getBlockEqs(b).begin(), it);
          }
          if (jblk == -1 && (it = dd.getBlockEqs(b).find(j+1)) != dd.getBlockEqs(b).end()) {
            jblk = b;
            eq2b[j][0] = b;
            eq2b[j][1] = glb2Blk[i][2] = std::distance(dd.getBlockEqs(b).begin(), it);
          }
        }
