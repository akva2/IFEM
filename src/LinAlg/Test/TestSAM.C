...
TEST(TestSAM, SingleBasisDirichlet1P)
{
  ...
  for (int i = 1; i <= 3; ++i) {
    ...
    ASSERT_EQ(sam->getEquation(3*(i-1)+3, 1), -i);
  }
}
...
TEST(TestSAM, MixedBasisDirichlet1P)
{
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    ASSERT_EQ(sam->getEquation(4*(i-1)+4, 1), -i);
  }
...
TEST(TestSAM, SingleBasisDirichlet2P)
{
  ...
  ASSERT_EQ(sam->getEquation(5, 1), -1);
  ASSERT_EQ(sam->getEquation(6, 1), -2);
}
...
TEST(TestSAM, MixedBasisDirichlet2P)
{
  ...
  for (int i = 1; i <= 3; ++i) {
    ...
    ASSERT_EQ(sam->getEquation(pch->getNodeID((i-1)*3+3), 1), -i);
  }
