...
bool ASMs2D::createProjectionBasis (bool init)
{
  ...
  surf = this->getBasis(ASM::GEOMETRY_BASIS);
  return true;
}
...
Field* ASMs2D::getProjectedField (const Vector& coefs) const
{
  if (this->getNoProjectionNodes() == coefs.size())
    return new SplineField2D(this->getBasis(ASM::PROJECTION_BASIS),coefs);
...
Fields* ASMs2D::getProjectedFields (const Vector& coefs, size_t) const
{
  if (this->getBasis(ASM::PROJECTION_BASIS) == this->getBasis(1) ||
      this->getNoProjectionNodes() == 0)
    return nullptr;

  size_t ncmp = coefs.size() / this->getNoProjectionNodes();
  if (ncmp*this->getNoProjectionNodes() == coefs.size())
    return new SplineFields2D(this->getBasis(ASM::PROJECTION_BASIS),coefs,ncmp);
