...
bool ASMs2D::checkRightHandSystem ()
{
  ...
  static_cast<Go::SplineSurface*>(geomB)->reverseParameterDirection(false);
				   ...
				   bool project)
{
  ...
  for (i = 0; i < gpar.size(); i++, k += 6)
  {
    ...
    u[tdir] = gpar[i];
    static_cast<const Go::SplineSurface*>(geomB)->point(pts,u[0],u[1],1);
			...
			const TimeDomain& time)
{
  ...
  std::vector<Go::BasisDerivsSf>  splineRed, splinegRed;
  if (use3rdDer)
  ...
  static_cast<const Go::SplineSurface*>(geomB)->computeBasisGrid(gpar[0],gpar[1],splineg);

  if (xr) {
    surf->computeBasisGrid(redpar[0],redpar[1],splineRed);
    static_cast<const Go::SplineSurface*>(geomB)->computeBasisGrid(redpar[0],redpar[1],splinegRed);
  }
...
#pragma omp parallel for schedule(static)
    for (size_t t = 0; t < groups[g].size(); t++)
    {
      ...
      fe.q = p2 - 1;
      Vector   Ng;
      Matrix   dNdu, dNgdu, Xnod, Jac, Xnodg;
      ...
      for (size_t e = 0; e < groups[g][t].size() && ok; e++)
      {
        ...
        if (integrand.getIntegrandType() & Integrand::ELEMENT_CENTER)
        {
          ...
          SplineUtils::point(X,param[0],param[1],static_cast<Go::SplineSurface*>(geomB));
          if (!useElmVtx)
          {
        ...
        if (xr)
        {
          ...
          for (int j = 0; j < nRed; j++, ip += nRed*(nel1-1))
            for (int i = 0; i < nRed; i++, ip++)
            {
              ...
              Vector Ng;
              Matrix dNgdu;
              SplineUtils::extractBasis(splinegRed[ip],Ng,dNgdu);
                        ...
                        const Real3DMat& itgPts)
{
  ...
  for (i = k = 0; i < itgPts.size(); i++)
    for (j = 0; j < itgPts[i].size(); j++, k++) {
      ...
      static_cast<const Go::SplineSurface*>(geomB)->computeBasis(itgPts[i][j][0],
                                                                 itgPts[i][j][1],
                                                                 splineg[k]);
    }
...
bool ASMs2D::tesselate (ElementBlock& grid, const int* npe) const
{
  ...
  static_cast<const Go::SplineSurface*>(geomB)->gridEvaluator(XYZ,gpar[0],gpar[1]);
