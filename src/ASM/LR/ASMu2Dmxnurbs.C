...
//==============================================================================
//!
//! \file ASMu2Dmxnurbs.C
//!
//! \date Apr 11 2023
//!
//! \author Arne Morten Kvarving / SINTEF
//!
//! \brief Driver for assembly of unstructured 2D nurbs mixed FE models.
//!
//==============================================================================

#include "ASMu2Dmxnurbs.h"
...
ASMu2Dmxnurbs::ASMu2Dmxnurbs (unsigned char n_s, const CharVec& n_f)
  : ASMu2Dnurbs(n_s, *std::max_element(n_f.begin(),n_f.end())), ASMmxBase(n_f)
{
  threadBasis = nullptr;
}


ASMu2Dmxnurbs::ASMu2Dmxnurbs (const ASMu2Dmxnurbs& patch, const CharVec& n_f)
  : ASMu2Dnurbs(patch), ASMmxBase(n_f[0]==0?patch.nfx:n_f),
...
const LR::LRSplineSurface* ASMu2Dmxnurbs::getBasis (int basis) const
{
...
LR::LRSplineSurface* ASMu2Dmxnurbs::getBasis (int basis)
{
  if (basis < 1 || basis > (int)m_basis.size())
    return nullptr;
...
bool ASMu2Dmxnurbs::read (std::istream& is, int basis)
{
  if (basis == 0)
    return this->ASMu2Dnurbs::read(is);
...
bool ASMu2Dmxnurbs::write (std::ostream& os, int basis) const
{
  if (basis == -1)
    os << *projBasis;
  else
...
void ASMu2Dmxnurbs::clear (bool retainGeometry)
{
  if (!retainGeometry) {
  ...
  // Erase the FE data
  this->ASMu2Dnurbs::clear(retainGeometry);
}


size_t ASMu2Dmxnurbs::getNoNodes (int basis) const
{
...
unsigned char ASMu2Dmxnurbs::getNoFields (int basis) const
{
...
unsigned char ASMu2Dmxnurbs::getNodalDOFs (size_t inod) const
{
...
char ASMu2Dmxnurbs::getNodeType (size_t inod) const
{
  if (this->isLMn(inod))
    return 'L';
  size_t nbc=nb.front();
...
void ASMu2Dmxnurbs::initMADOF (const int* sysMadof)
{
  this->initMx(MLGN,sysMadof);
}


void ASMu2Dmxnurbs::extractNodeVec (const RealArray& globRes,
                                    RealArray& nodeVec,
...
bool ASMu2Dmxnurbs::injectNodeVec (const RealArray& nodeRes,
                                   RealArray& globRes,
                                   unsigned char, int basis) const
{
...
bool ASMu2Dmxnurbs::getSolution (Matrix& sField, const Vector& locSol,
...
bool ASMu2Dmxnurbs::generateFEMTopology ()
{
  if (!myMLGN.empty())
    return true;
  ...
  if (tensorPrjBas)
  {
    projBasis.reset(createLRnurbs(tensorPrjBas));
    delete tensorPrjBas;
  ...
  if (m_basis.empty()) {
    ...
    for (size_t b = 0; b < svec.size(); b++)
      m_basis[b].reset(createLRnurbs(svec[b].get()));
    ...
    if (ASMmxBase::Type == ASMmxBase::REDUCED_CONT_RAISE_BASIS1 ||
      ...
      if (ASMmxBase::Type == ASMmxBase::SUBGRID) {
        refBasis.reset(createLRnurbs(otherBasis));
        if (!projBasis)
        ...
        if (!projBasis)
          projBasis.reset(createLRnurbs(otherBasis));
        refBasis = projBasis;
      }
      delete otherBasis;
    }
...
bool ASMu2Dmxnurbs::integrate (Integrand& integrand,
    ...
    for (size_t e = 0; e < groups[t].size(); ++e)
    {
      ...
      for (int j = 0; j < nGauss; j++)
        for (int i = 0; i < nGauss; i++, fe.iGP++)
        {
          ...
          if (use2ndDer) {
            if (!utl::Hessian(Hess,fe.hess(elmBasis),Jac,Xnod,
                              d2Nxdu2[elmBasis-1],fe.grad(elmBasis),true))
              ok = false;
...
bool ASMu2Dmxnurbs::integrate (Integrand& integrand, int lIndex,
                               GlobalIntegral& glInt,
                               const TimeDomain& time)
{
  ...
  PROFILE2("ASMu2Dmxnurbs::integrate(B)");
  ...
  std::vector<LR::Element*>::iterator el1 = m_basis[elmBasis-1]->elementBegin();
  for (int iel = 1; el1 != m_basis[elmBasis-1]->elementEnd(); ++el1, ++iel)
  {
                               ...
                               const ASM::InterfaceChecker& iChkgen)
{
  ...
  PROFILE2("ASMu2Dmxnurbs::integrate(J)");
        ...
        for (size_t i = 0; i < intersections.size() && ok; ++i) {
          ...
          for (int g = 0; g < nGP && ok; g++, ++fe.iGP)
          {
            ...
            fe.detJxW = utl::Jacobian(Jac2, normal,
                                      fe.grad(elmBasis+m_basis.size()),
                                      Xnod2,dNxdu[elmBasis-1+m_basis.size()],t1,t2);
            fe.detJxW = utl::Jacobian(Jac, normal,
                                      fe.grad(elmBasis),Xnod,dNxdu[elmBasis-1],t1,t2);
            if (fe.detJxW == 0.0) continue; // skip singular points
            for (size_t b = 0; b < m_basis.size(); ++b)
              if (b != static_cast<size_t>(elmBasis)-1) {
                fe.grad(b+1).multiply(dNxdu[b],Jac);
...
bool ASMu2Dmxnurbs::evalSolution (Matrix& sField, const Vector& locSol,
...
bool ASMu2Dmxnurbs::evalSolution (Matrix& sField,
...
bool ASMu2Dmxnurbs::refine (const LR::RefineData& prm, Vectors& sol)
{
  if (shareFE)
    return true;
...
Vec3 ASMu2Dmxnurbs::getCoord (size_t inod) const
{
  size_t b = 0;
  size_t nbb = 0;
...
void ASMu2Dmxnurbs::generateThreadGroups (const Integrand& integrand, bool silence,
                                          bool ignoreGlobalLM)
{
  int p1 = 0;
...
bool ASMu2Dmxnurbs::connectPatch (int edge, ASM2D& neighbor, int nedge, bool revers,
                                  int basis, bool coordCheck, int thick)
{
  ASMu2Dmxnurbs* neighMx = dynamic_cast<ASMu2Dmxnurbs*>(&neighbor);
...
void ASMu2Dmxnurbs::getBoundaryNodes (int lIndex, IntVec& nodes, int basis,
                                      int thick, int orient, bool local) const
{
  if (basis > 0)
    this->ASMu2Dnurbs::getBoundaryNodes(lIndex, nodes, basis, thick, orient, local);
  else
    for (size_t b = 1; b <= this->getNoBasis(); ++b)
      this->ASMu2Dnurbs::getBoundaryNodes(lIndex, nodes, b, thick, orient, local);
}


void ASMu2Dmxnurbs::remapErrors (RealArray& errors,
...
size_t ASMu2Dmxnurbs::getNoRefineNodes() const
{
  return refBasis->nBasisFunctions();
}


size_t ASMu2Dmxnurbs::getNoRefineElms() const
{
...
void ASMu2Dmxnurbs::copyRefinement (LR::LRSplineSurface* basis,
...
void ASMu2Dmxnurbs::swapProjectionBasis ()
{
  if (altProjBasis) {
