...
bool ASMu3D::transferGaussPtVars (const LR::LRSpline* oldBasis1,
                                  const RealArray& oldVars, RealArray& newVars,
                                  int nGauss) const
{
  const LR::LRSplineVolume* newBasis = this->getBasis();
  const LR::LRSplineVolume* oldBasis = static_cast<const LR::LRSplineVolume*>(oldBasis1);
  ...
  for (int iEl = 0; iEl < newBasis->nElements(); iEl++)
  {
    ...
    for (int i = 0; i < nGauss; ++i)
      for (int j = 0; j < nGauss; ++j)
        for (int k = 0; k < nGauss; ++k)
          data(j + 1, i + k*(nGauss)) = I1(i, j + k*(nGauss));
    I1 = I[1]*data;
    for (int i = 0; i < nGauss; ++i)
      for (int j = 0; j < nGauss; ++j)
        for (int k = 0; k < nGauss; ++k)
          data(k + 1, i + j*(nGauss)) = I1(j+1, i + k*(nGauss));

    I1 = I[2]*data;
    for (int i = 0; i < nGauss; ++i)
      for (int j = 0; j < nGauss; ++j)
        for (int k = 0; k < nGauss; ++k)
          newVars[iEl*nGp+i+(j + k*nGauss)*nGauss] = I1(k+1, i + j*nGauss);
  }
...
bool ASMu3D::transferGaussPtVarsN (const LR::LRSpline* oldBasis1,
                                   const RealArray& oldVars, RealArray& newVars,
                                   int nGauss) const
{
  const LR::LRSplineVolume* newBasis = this->getBasis();
...
bool ASMu3D::transferCntrlPtVars (const LR::LRSpline* oldBasis1,
