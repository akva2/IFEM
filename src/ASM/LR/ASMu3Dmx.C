...
bool ASMu3Dmx::read (std::istream& is, int basis)
{
  ...
  if (basis == 0) // NOTE: the use of 0 conflicts with GEOMETRY_BASIS!
    return this->ASMu3D::read(is);
  else if (basis < 0) {
    std::cerr << "*** ASMu3Dmx::read: Asked to read basis " << basis
