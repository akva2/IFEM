  ...
  : Fields(name), basis(patch->getBasis(nbasis)), vol(patch->getVolume())
{
  ...
  if (patch->getNoFields(nbasis) != 3) {
    RealArray::const_iterator end = v.size() > nno*patch->getNoFields(nbasis)+ofs ? vit+nno*nf : v.end();
...
bool LRSplineFields3D::valueFE (const FiniteElement& fe, Vector& vals) const
{
  ...
  auto elm = basis->getElement(basis->getElementContaining(fe.u,fe.v,fe.w));
...
bool LRSplineFields3D::gradFE(const FiniteElement& fe, Matrix& grad) const
{
  ...
  auto elm = vol->getElement(vol->getElementContaining(fe.u,fe.v,fe.w));
  ...
  if (basis != vol)
  {
    ...
    auto belm = basis->getElement(basis->getElementContaining(fe.u,fe.v,fe.w));
