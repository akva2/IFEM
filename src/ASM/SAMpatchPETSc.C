...
DofIS& SAMpatchPETSc::getIS (char dofType) const
{
  ...
  for (int n : adm.dd.getMLGN()) {
    // TODO: Fix this in case we have dof_type instead of nodeType
    if ((dofType == 'A' || nodeType.empty() || this->SAM::getNodeType(inod) == dofType) &&
        n >= adm.dd.getMinNode() && n <= adm.dd.getMaxNode()) {
      std::pair<int, int> dofs = this->SAM::getNodeDOFs(inod);
