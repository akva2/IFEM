...
//==============================================================================
//!
//! \file ASMLRSplineGo.C
//!
//! \date Apr 7 2025
//!
//! \author Arne Morten Kvarving / SINTEF
//!
//! \brief Base class for unstructured Go spline-based FE assembly drivers.
//!
//==============================================================================

#include "ASMLRSplineGo.h"
...
int GoLR::extendControlPoints (Go::LRSplineSurface* basis, const Vector& v, int nf, int ofs)
{
  if (v.empty())
    return 0;
  else if (v.size() == static_cast<size_t>(basis->numBasisFunctions()))
    nf = 1; // This is a scalar field
  else if (v.size() != static_cast<size_t>(basis->numBasisFunctions()*nf)) {
    std::cerr <<" *** GoLR::extendControlPoints: Invalid vector size "
              << v.size() <<", nBasis = "<< basis->numBasisFunctions()
              << std::endl;
    return -1;
  }

  RealArray cpts;
  const auto allBasis = std::array{basis->basisFunctionsBegin(), basis->basisFunctionsEnd()};
  int id = 0;
  for (const auto b : allBasis) {
    const auto cpp = b->second->Coef();
    int istart = ofs + id*nf;
...
void GoLR::contractControlPoints (Go::LRSplineSurface* basis, Vector& v, int nf, int ofs)
{
  RealArray cpts;
  const auto allBasis = std::array{basis->basisFunctionsBegin(), basis->basisFunctionsEnd()};
  int id = 0;
...
void GoLR::getGaussPointParameters (const Go::LRSplineSurface* lrspline, RealArray& uGP,
                                    int d, int nGauss, int iel, const double* xi)
{
#ifdef INDEX_CHECK
  if (iel < 1 || iel > lrspline->numElements())
  {
    std::cerr <<" *** GoLR::getGaussPointParameters: Element index "<< iel
  ...
  std::advance(el, iel-1);
  double ustart = d == 0 ? el->second->umin() :  el->second->vmin();
  double ustop  = d == 0 ? el->second->umax() :  el->second->vmax();
...
void GoLR::generateThreadGroups (ThreadGroups& threadGroups,
                                 const Go::LRSplineSurface* lr,
                                 const std::vector<Go::LRSplineSurface*>& addConstraints)
{
  int nElement = lr->numElements();
#ifdef USE_OPENMP
  ...
  //     const auto allElms = std::array{lr->elementsBegin(), lr->elementsEnd()};
  //     for (auto e : allElms) {
  //       for (Go::LRSplineSurface* lr2 : addConstraints) {
  //          const auto elB = lr2->coveringElement((e->second->umin() + e->second->umax()) / 2,
  //                                                 (e->second->vmin() + e->second->vmax()) / 2);
  //          const auto supp = std::array{elB->supportBegin(), elB->supportEnd()};
  //          for (const auto b2 : supp)
...
ASMLRSplineGo::ASMLRSplineGo (unsigned char n_p, unsigned char n_s,
                              unsigned char n_f)
  : ASMbase(n_p,n_s,n_f)
{
  geomB = nullptr;
...
ASMLRSplineGo::ASMLRSplineGo (const ASMLRSplineGo& patch, unsigned char n_f)
  : ASMbase(patch,n_f)
{
  geomB = patch.geomB;
  projB = patch.projB;
...
bool ASMLRSplineGo::refine (const LR::RefineData& prm, Vectors& sol)
{
  if (!geomB)
    return false;
  ...
  if (shareFE && !prm.refShare)
  {
    ...
    nnod = geomB->numBasisFunctions();
    nel  = geomB->numElements();
    return true;
  }
...
// bool ASMLRSplineGo::doRefine (const LR::RefineData& prm, LR::LRSpline* lrspline)
// {
...
Go::BsplineBasis ASMLRSplineGo::getBezierBasis (int p, double start, double end)
{
...
IntVec ASMLRSplineGo::getFunctionsForElements (const IntVec& elements,
                                               bool globalId) const
{
...
void ASMLRSplineGo::getFunctionsForElements (IntSet& functions,
...
IntVec ASMLRSplineGo::getBoundaryCovered (const IntSet& nodes) const
{
  IntSet result;
...
IntVec ASMLRSplineGo::getOverlappingNodes (const IntSet& nodes, int dir) const
{
  IntSet result;
  // for (int i : nodes)
  // {
...
// void ASMLRSplineGo::Sort (int u, int v, int orient,
//                           std::vector<Go::LRBSpline2D*>& functions)
// {
...
bool ASMLRSplineGo::transferCntrlPtVars (Go::LRSplineSurface* oldBasis,
...
std::pair<size_t,double> ASMLRSplineGo::findClosestNode (const Vec3& X) const
{
  double distance = 0.0;
  size_t inod = 0, iclose = 0;
  const auto allFunc = std::array{geomB->basisFunctionsBegin(), geomB->basisFunctionsEnd()};
  for (const auto& b : allFunc)
  {
    const double d = (X - Vec3(b->second->Coef().begin(), nsd)).length();
...
Vec3 ASMLRSplineGo::getElementCenter (int iel) const
{
#ifdef INDEX_CHECK
  if (iel < 1 || iel > geomB->numElements())
  {
    std::cerr <<" *** ASMLRSplineGo::getElementCenter: Element index "<< iel
              <<" out of range [1,"<< geomB->numElements() <<"]."<< std::endl;
    return Vec3();
  }
#endif

  auto parmin = [](const auto& elm, unsigned char d)
  {
    return d == 0 ? elm->second->umin() : elm->second->vmin();
  };
  auto parmax = [](const auto& elm, unsigned char d)
  {
    return d == 0 ? elm->second->umax() : elm->second->vmax();
  };

  double u0[3] = { 0.0, 0.0, 0.0 };
  auto elm = geomB->elementsBegin();
  std::advance(elm, iel-1);
  for (unsigned char d = 0; d < ndim; d++)
    u0[d] = 0.5*(parmin(elm, d) + parmax(elm, d));
...
bool ASMLRSplineGo::checkThreadGroups (const IntMat& groups,
...
void ASMLRSplineGo::analyzeThreadGroups (const IntMat& groups)
{
...
bool ASMLRSplineGo::getParameterDomain (Real2DMat& u, IntVec* corners) const
{
...
void ASMLRSplineGo::getNoIntPoints (size_t& nPt, size_t& nIPt)
{
  size_t nGp = 1;
...
void ASMLRSplineGo::swapProjectionBasis ()
{
  if (projB2) {
