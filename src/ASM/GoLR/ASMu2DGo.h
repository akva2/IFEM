...
#ifndef _ASM_U2D_GO_H
#define _ASM_U2D_GO_H

#include "ASM2D.h"
#include "ASMbase.h"
#include "GoLR/ASMLRSplineGo.h"
...
class ASMu2DGo : public ASMLRSplineGo, public ASM2D
{
protected:
  ...
  public:
    ...
    BasisFunctionCache(const ASMu2DGo& pch);
  ...
  protected:
    const ASMu2DGo& patch; //!< Reference to patch cache is for
...
public:
  ...
  public:
    ...
    explicit InterfaceChecker(const ASMu2DGo& pch);
    //! \brief Empty destructor.
  ...
  protected:
    const ASMu2DGo& myPatch; //!< Reference to the patch being integrated
  ...
  ASMu2DGo(unsigned char n_s = 2, unsigned char n_f = 2);
  //! \brief Copy constructor.
  ASMu2DGo(const ASMu2DGo& patch, unsigned char n_f = 0);
  //! \brief Empty destructor.
  virtual ~ASMu2DGo() { geomB = nullptr; }

  //! \brief Returns the spline surface representing a basis of this patch.
  virtual const Go::LRSplineSurface* getBasis(int basis = 1) const;
  //! \brief Returns the spline surface representing a basis of this patch.
  virtual Go::LRSplineSurface* getBasis(int basis = 1);
  ...
  void copyRefinement(Go::LRSplineSurface* basis, int multiplicity = 1) const;
...
public:
  ...
  void setOutputMaster(const ASMu2DGo* pch)
  { outputMaster = pch; }
...
protected:
  ...
  {
    Go::LRSplineSurface* lr;       //!< Pointer to the right object (in case of multiple bases)
    int                 edg;       //!< Which edge is this
    ...
    DirichletEdge(Go::LRSplineSurface* sf, int dir,
  ...
  //! \param[in] integrand Object with problem-specific data and methods
  Go::LRSplineSurface* projectSolution(const IntegrandBase& integrand) const;
  //! \brief Projects the secondary solution using a superconvergent approach.
  //! \param[in] integrand Object with problem-specific data and methods
  Go::LRSplineSurface* scRecovery(const IntegrandBase& integrand) const;
  ...
  Go::LRSplineSurface* regularInterpolation(const RealArray& upar,
...
public:
  //! \brief Projects the secondary solution field onto the primary basis.
  //! \param[in] integrand Object with problem-specific data and methods
  virtual Go::LRSplineSurface* evalSolution(const IntegrandBase& integrand) const;
  ...
  virtual bool transferGaussPtVars(const Go::LRSplineSurface* old_basis,
  ...
  //! \param[out] newVar Gauss point variables associated with this patch
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferGaussPtVarsN(const Go::LRSplineSurface* old_basis,
                                    const RealArray& oldVar, RealArray& newVar,
                                    int nGauss) const;

  using ASMLRSplineGo::transferCntrlPtVars;
  //! \brief Transfers control point variables from old basis to this patch.
  //! \param[in] old_basis The LR-spline basis to transfer from
  //! \param[out] newVar Gauss point variables associated with this patch
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferCntrlPtVars(const Go::LRSplineSurface* old_basis,
...
protected:
  ...
  //! \param[in] coordCheck False to disable coordinate checks
  //! \param[in] thick Thickness of connection
  bool connectBasis(int edge, ASMu2DGo& neighbor, int nedge, bool revers,
  ...
  void getGaussPointParameters(RealArray& uGP, int dir, int nGauss,
                               ...
                               const Go::LRSplineSurface* spline = nullptr) const;
  ...
  std::shared_ptr<Go::LRSplineSurface> createLRfromTensor();

  //! \brief Converts a rational spline surface to a LR NURBS surface.
  static std::shared_ptr<Go::LRSplineSurface> createLRNurbs(const Go::SplineSurface& srf);
  ...
  void writePostscriptElementsNurbs (std::shared_ptr<Go::LRSplineSurface> mesh,
                                     std::ostream& out, bool close = true,
                                     int nu = 2, int nv = 2);
  //! \brief Write NURBS elements as postscript file.
  void writePostscriptMeshWithControlPointsNurbs (std::shared_ptr<Go::LRSplineSurface> mesh,
...
public:
  ...
  static void computeBasisNurbs(double u, double v,
                                Go::BasisPtsSf& bas, int iel,
                                const Go::LRSplineSurface& spline);
...
protected:
  std::shared_ptr<Go::LRSplineSurface> lrspline; //!< The LR-spline surface object

  bool is_rational = false; //!< True if basis is rational

  const ASMu2DGo* outputMaster = nullptr; //!< Master patch to use for VTF output
