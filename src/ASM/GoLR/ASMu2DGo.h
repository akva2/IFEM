...
//!
//! \brief Driver for assembly of unstructured 2D spline FE models using GoTools.
//!
//==============================================================================
...
public:
  ...
  virtual const Go::LRBSpline2D* getBasis(int basis = 1) const;
  //! \brief Returns the spline surface representing a basis of this patch.
  virtual Go::LRBSpline2D* getBasis(int basis = 1);
  ...
  void copyRefinement(Go::LRBSpline2D* basis, int multiplicity = 1) const;
...
protected:
  ...
  struct DirichletEdge
  {
    ...
    DirichletEdge(Go::LRBSpline2D* sf, int dir,
  ...
  //! \param[in] integrand Object with problem-specific data and methods
  Go::LRBSpline2D* projectSolution(const IntegrandBase& integrand) const;
  //! \brief Projects the secondary solution using a superconvergent approach.
  //! \param[in] integrand Object with problem-specific data and methods
  Go::LRBSpline2D* scRecovery(const IntegrandBase& integrand) const;
  ...
  Go::LRBSpline2D* regularInterpolation(const RealArray& upar,
...
public:
  //! \brief Projects the secondary solution field onto the primary basis.
  //! \param[in] integrand Object with problem-specific data and methods
  virtual Go::LRBSpline2D* evalSolution(const IntegrandBase& integrand) const;
  ...
  virtual bool transferGaussPtVars(const Go::LRBSpline2D* old_basis,
  ...
  //! \param[out] newVar Gauss point variables associated with this patch
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferGaussPtVarsN(const Go::LRBSpline2D* old_basis,
                                    const RealArray& oldVar, RealArray& newVar,
                                    int nGauss) const;
  ...
  //! \param[out] newVar Gauss point variables associated with this patch
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferCntrlPtVars(const Go::LRBSpline2D* old_basis,
...
protected:
  ...
  void getGaussPointParameters(RealArray& uGP, int dir, int nGauss,
                               ...
                               const Go::LRBSpline2D* spline = nullptr) const;
  ...
  std::shared_ptr<Go::LRBSpline2D> createLRfromTensor();

  //! \brief Converts a rational spline surface to a LR NURBS surface.
  static std::shared_ptr<Go::LRBSpline2D> createLRNurbs(const Go::SplineSurface& srf);
  ...
  void writePostscriptElementsNurbs (std::shared_ptr<Go::LRBSpline2D> mesh,
                                     std::ostream& out, bool close = true,
                                     int nu = 2, int nv = 2);
  //! \brief Write NURBS elements as postscript file.
  void writePostscriptMeshWithControlPointsNurbs (std::shared_ptr<Go::LRBSpline2D> mesh,
...
protected:
  std::shared_ptr<Go::LRBSpline2D> lrspline; //!< The LR-spline surface object
