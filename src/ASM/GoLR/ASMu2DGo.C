...
//!
//! \brief Driver for assembly of unstructured 2D Go spline FE models.
//!
//==============================================================================
...
ASMu2DGo::ASMu2DGo (unsigned char n_s, unsigned char n_f)
  : ASMLRSplineGo(2,n_s,n_f), lrspline(nullptr),
...
ASMu2DGo::ASMu2DGo (const ASMu2DGo& patch, unsigned char n_f)
  : ASMLRSplineGo(patch,n_f), lrspline(patch.lrspline),
    bezierExtract(patch.myBezierExtract)
{
  ...
  if (nnod == 0 && lrspline)
    nnod = lrspline->numBasisFunctions();
}


const Go::LRSplineSurface* ASMu2DGo::getBasis (int basis) const
{
  switch (basis) {
    case ASM::GEOMETRY_BASIS:
      return static_cast<const Go::LRSplineSurface*>(geomB.get());
    case ASM::PROJECTION_BASIS:
      return static_cast<const Go::LRSplineSurface*>(projB.get());
    case ASM::PROJECTION_BASIS_2:
      return static_cast<const Go::LRSplineSurface*>(projB2.get());
    case ASM::REFINEMENT_BASIS:
      return static_cast<const Go::LRSplineSurface*>(refB.get());
    default:
      return lrspline.get();
  }
}
...
Go::LRSplineSurface* ASMu2DGo::getBasis (int basis)
{
  if (tensorspline)
    this->createLRfromTensor();

  return const_cast<Go::LRSplineSurface*>(std::as_const(*this).getBasis(basis));
}


bool ASMu2DGo::read (std::istream& is)
{
  ...
  if (strncmp(firstline, "# LRSPLINE", 10) == 0) {
    lrspline.reset(new Go::LRSplineSurface());
    is >> *lrspline;
  }
  else {
    ...
    is >> *tensorspline;
    //lrspline.reset(new Go::LRSplineSurface(tensorspline));
  }
  ...
  if (!is.good() && !is.eof())
  {
    std::cerr <<" *** ASMu2DGo::read: Failure reading spline data"<< std::endl;
    lrspline.reset();
    return false;
  }
  else if (readDim < 2)
  {
    std::cerr <<" *** ASMu2DGo::read: Invalid lrspline patch, dim="
              << readDim << std::endl;
    lrspline.reset();
    return false;
  }
  ...
  {
    std::cout <<"  ** ASMu2DGo::read: The dimension of this lrspline patch "
...
bool ASMu2DGo::write (std::ostream& os, int basis) const
{
  if (!lrspline) return false;
  if (basis > static_cast<int>(this->getNoBasis())) return false;
  const Go::LRSplineSurface* spline = this->getBasis(basis);
...
void ASMu2DGo::clear (bool retainGeometry)
{
  if (!retainGeometry) {
    // Erase spline data
    if (!shareFE) {
...
bool ASMu2DGo::cornerRefine (int minBasisfunctions)
{
  if (!lrspline) return false;
  if (shareFE) return true;

  // double h = 1.0;
  // int nBasis = lrspline->numBasisFunctions();
  // double unif_step_h = 1.0 / ((minBasisfunctions - nBasis) / 3.0 + 1.0);
  // while(lrspline->numBasisFunctions() < minBasisfunctions) {
...
bool ASMu2DGo::diagonalRefine (int minBasisfunctions)
{
  if (!lrspline) return false;
  if (shareFE) return true;
...
bool ASMu2DGo::uniformRefine (int minBasisfunctions)
{
  if (!lrspline) return false;
  if (shareFE) return true;
...
bool ASMu2DGo::uniformRefine (int dir, int nInsert)
{
...
bool ASMu2DGo::refine (int dir, const RealArray& xi, double scale)
{
...
double ASMu2DGo::getMinimumSize (int nrefinements) const
{
  if (aMin > 0.0 || lrspline->numElements() <= 0)
    return aMin;

  double redMax = pow(2.0,nrefinements);
  aMin = lrspline->elementsBegin()->second->area()/(redMax*redMax);
  return aMin;
}


bool ASMu2DGo::checkElementSize (int elmId, bool globalNum) const
{
  if (globalNum)
    elmId = utl::findIndex(MLGE,1+elmId);

  if (elmId >= 0 && elmId < lrspline->numElements()) {
    auto el = lrspline->elementsBegin();
    std::advance(el, elmId);
    return el->second->area() > aMin+1.0e-12;
  } else
    return false;
}


void ASMu2DGo::extendRefinementDomain (IntSet& refineIndices,
...
bool ASMu2DGo::raiseOrder (int ru, int rv)
{
...
bool ASMu2DGo::createProjectionBasis (bool init)
{
  if (!tensorspline)
    return false;
  ...
  lrspline = std::static_pointer_cast<Go::LRSplineSurface>(geomB);
  return true;
}


bool ASMu2DGo::evaluateBasis (int iel, FiniteElement& fe, int derivs) const
{
  if (is_rational)
...
#ifdef INDEX_CHECK
  if (iel < 0 || iel >= lrspline->numElements())
  {
    std::cerr <<" *** ASMu2D::evaluateBasis: Element index "<< 1+iel
              <<" out of range [1,"<< lrspline->numElements() <<"]."<< std::endl;
    return false;
  }
#endif
  ...
  auto el = lrspline->elementsBegin();
  std::advance(el, iel);
  fe.xi  = 2.0*(fe.u - el->second->umin()) / (el->second->umax() - el->second->umin()) - 1.0;
  fe.eta = 2.0*(fe.v - el->second->vmin()) / (el->second->vmax() - el->second->vmin()) - 1.0;
  RealArray Nu, Nv;
#pragma omp critical
  {
  ...
  Vector B(lrspline->degree(Go::Direction2D::XFIXED) * lrspline->degree(Go::Direction2D::YFIXED)); // Bezier basis functions
...
std::shared_ptr<Go::LRSplineSurface>
ASMu2DGo::createLRNurbs (const Go::SplineSurface& srf)
{
  return std::make_shared<Go::LRSplineSurface>(srf.numCoefs_u(), srf.numCoefs_v(),
...
ASMu2DGo::createLRfromTensor ()
{
  if (tensorspline)
  {
    ...
    else if (tensorspline->dimension() > nsd)
    {
      ...
      lrspline.reset(new Go::LRSplineSurface(tensorspline->order_u(),
                                             tensorspline->order_v(),
                                             tensorspline->numCoefs_u(),
...
bool ASMu2DGo::generateFEMTopology ()
{
  ...
  if (tensorPrjBas)
  {
    projB = tensorPrjBas->rational() ? createLRNurbs(*tensorPrjBas)
                                     : std::make_shared<Go::LRSplineSurface>(tensorPrjBas);
    //projB->generateIDs();
    delete tensorPrjBas;
  ...
  size_t iel = 0;
  const auto allElem = std::array{lrspline->elementsBegin(), lrspline->elementsEnd()};
  for (const auto elm : allElem)
  {
    myMLGE[iel] = ++gEl; // global element number over all patches
    myMNPC[iel].resize(elm->second->getSupport().size());

    int lnod = 0;
    for (auto b : elm->second->getSupport()) {
      const auto it = std::find(lrspline->basisFunctionsBegin(),
                                lrspline->basisFunctionsEnd(),
                                b);
      const auto id = std::distance(lrspline->basisFunctionsBegin(), it);
      myMNPC[iel][lnod++] = id;
    }
    ++iel;
  }
...
bool ASMu2DGo::connectPatch (int edge, ASM2D& neighbor, int nedge,
                             bool revers, int, bool coordCheck, int thick)
{
  ASMu2DGo* neighU = dynamic_cast<ASMu2DGo*>(&neighbor);
  if (!neighU)
    return false;
...
bool ASMu2DGo::connectBasis (int edge, ASMu2DGo& neighbor, int nedge, bool revers,
                             ...
                             bool coordCheck, int thick)
{
  ...
  {
    std::cerr <<" *** ASMu2DGo::connectBasis: Logic error, cannot"
  ...
  {
    std::cerr <<" *** ASMu2DGo::connectBasis: Non-matching edges, sizes "
  ...
  for (size_t i = 0; i < masterNodes.size(); ++i)
  {
    ...
    {
      std::cerr <<" *** ASMu2DGo::connectBasis: Non-matching nodes "
...
ASMu2DGo::DirichletEdge::DirichletEdge (Go::LRSplineSurface* sf,
  ...
  : lr(sf), edg(LR::NONE), dof(d), code(c)
{
  ...
  std::vector<Go::LRBSpline2D*> c1, c2;
  switch (dir)
  {
  case -2:
    edg = LR::SOUTH;
    c1 = lr->getBoundaryBsplines(Go::Direction2D::YFIXED, false);
    c2 = lr->getBoundaryBsplines(Go::Direction2D::YFIXED, true);
    break;
  case -1:
    edg = LR::WEST;
...
void ASMu2DGo::constrainEdge (int dir, bool open, int dof, int code, char basis)
{
  if (basis < 1) basis = 1;
  ...
  const auto edgeFunctions = de.lr->getBoundaryBsplines(de.edg == 1 || de.edg == 2 ? Go::XFIXED : Go::YFIXED, de.edg % 2 == 0);
