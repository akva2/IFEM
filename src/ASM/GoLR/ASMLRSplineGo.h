...
namespace LR //! Utilities for LR-splines.
{
  ...
  int extendControlPoints(Go::LRBSpline2D* basis, const Vector& v,
                          int nf, int ofs = 0);
  ...
  //! \param[in] nf Number of fields in the given vector
  //! \param[in] ofs Offset in vector
  void contractControlPoints(Go::LRBSpline2D* basis, Vector& v,
                             int nf, int ofs = 0);
  ...
  void getGaussPointParameters(const Go::LRBSpline2D* spline, RealArray& uGP,
...
public:
  ...
  const Go::LRBSpline2D* getRefinementBasis() const { return refB.get(); }
  ...
  virtual Go::LRBSpline2D* evalSolution(const IntegrandBase& integrand) const = 0;
  ...
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferGaussPtVars(const Go::LRBSpline2D* oldBasis,
  ...
  //! \param[out] newVar Gauss point variables associated with this patch
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferGaussPtVarsN(const Go::LRBSpline2D* oldBasis,
                                    const RealArray& oldVar, RealArray& newVar,
                                    int nGauss) const = 0;
  ...
  //! \param[out] newVar Gauss point variables associated with this patch
  //! \param[in] nGauss Number of Gauss points along a knot-span
  virtual bool transferCntrlPtVars(const Go::LRBSpline2D* oldBasis,
  ...
  bool transferCntrlPtVars(Go::LRBSpline2D* oldBasis,
                           const RealArray& oldVar, RealArray& newVar,
...
protected:
  ...
  bool doRefine(const LR::RefineData& prm, Go::LRBSpline2D* lrspline);
  ...
  static bool checkThreadGroups(const IntMat& groups,
                                const std::vector<const Go::LRBSpline2D*>& bases,
                                const Go::LRBSpline2D* threadBasis);

  //! \brief Analyze and print thread group statistics.
  //! \param[in] groups The generated thread groups
  static void analyzeThreadGroups(const IntMat& groups);

  std::shared_ptr<Go::LRBSpline2D> geomB;  //!< Pointer to spline object of the geometry basis
  std::shared_ptr<Go::LRBSpline2D> projB;  //!< Pointer to spline object of the projection basis
  std::shared_ptr<Go::LRBSpline2D> projB2; //!< Pointer to spline object of the secondary projection basis
  std::shared_ptr<Go::LRBSpline2D> refB;   //!< Pointer to spline object of the refinement basis
