                                        ...
                                        bool neumann) const
{
  if (problem)
    ...
    return new L2Mats(*const_cast<GlbL2*>(this),nen,functions.front()->dim());
  else
    return nullptr;
}
...

{
  ...
  for (size_t b = 0; b < solPt.size(); b++)
    gl2.b[b].add(fe.N,solPt[b]*fe.detJxW);
...

{
  ...
  return const_cast<GlbL2*>(this)->formL2Mats(gl2.mnpc,solPt,fe,X);
}
