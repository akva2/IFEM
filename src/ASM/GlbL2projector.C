                                  ...
                                  bool continuous, bool enforceEnds) const
{
  ...
  PROFILE2("ASMbase::globalL2");
    ...
    const ProcessAdm& adm =
      integrand.getAdm().dd.isPartitioned() ? integrand.getAdm() : singleAdm;
#endif
  ...
  case LinAlg::UMFPACK:
    ...
    static_cast<SparseMatrix*>(A)->redim(npnod,npnod);
  ...
  case LinAlg::PETSC:
    if (GlbL2::SolverParams)
    {
      ...
      if (adm.dd.isPartitioned() && !useAdmPart && adm.getProcId() == 0) {
        ...
        this->getElmConnectivities(neighs, true);
      }
      Ap->init(npnod, &nodes, &neighs, useAdmPart ? &adm.dd.getElms() : nullptr);
      if (adm.dd.isPartitioned())
        const_cast<ASMbase*>(this)->generateProjThreadGroupsFromElms(Ap->getInternalDD().getElms());
