...
bool MxFiniteElement::Jacobian (Matrix& Jac, const Matrix& Xnod,
                                ...
                                const BasisValuesPtrs& bf)
{
  const bool separateGeometry = bf.size() > this->getNoBasis();
  if (separateGeometry)
    gBasis = bf.size();
  ...
  for (size_t b = 1; b <= this->getNoBasis(); b++)
    if (b != gBasis || separateGeometry)
      this->grad(b).multiply(bf[b-1]->dNdu, Jac);
                                ...
                                const Matrix* Xnod2)
{
  ...
  if (2*nBasis <= this->getNoBasis())
  {
    ...
    detJxW = utl::Jacobian(Jac,n,this->grad(nBasis+gBasis),
                           Xnod2 ? *Xnod2 : Xnod,
                           dNxdu[nBasis+gBasis-1]->dNdu,t1,t2);

    for (size_t b = 1; b <= nBasis; ++b)
      if (b != gBasis)
        this->grad(nBasis+b).multiply(dNxdu[nBasis+b-1]->dNdu,Jac);
  }
  else if (separateGeometry)
    nBasis = this->getNoBasis();

  Matrix& dX = separateGeometry ? dummy : this->grad(gBasis);
  detJxW = utl::Jacobian(Jac,n,dX,Xnod,dNxdu[gBasis-1]->dNdu,t1,t2);

  for (size_t b = 1; b <= nBasis; ++b)
    if (b != gBasis || separateGeometry)
      this->grad(b).multiply(dNxdu[b-1]->dNdu,Jac);
                               ...
                               const BasisValuesPtrs& bf)
{
  ...
  bool ok;
  if (separateGeometry)
    ok = Hess.multiply(Xnod, bf.back()->d2Ndu2);
  else
