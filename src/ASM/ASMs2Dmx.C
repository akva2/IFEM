                                   ...
                                   bool continuous) const
{
  ...
  for (int i2 = 0; i2 < nel2; i2++)
	...
	for (int i = 0; i < ng1; i++, ip++)
	{
	  ...
	  for (size_t ii = 0; ii < phi2.size(); ii++)
	  {
	    int inod = MNPC[iel][ii]+1;
	    for (size_t jj = 0; jj < phi2.size(); jj++)
	    {
	      int jnod = MNPC[iel][jj]+1;
	      A(inod,jnod) += phi2[ii]*phi2[jj]*dJw;
	    }
	    for (size_t r = 1; r <= sField.rows(); r++)
	      B(inod+(r-1)*nnod) += phi2[ii]*sField(r,ip+1)*dJw;
	  }
	}
    }
