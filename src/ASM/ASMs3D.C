...
void ASMs3D::getElmConnectivities (NeighArray& neigh) const
{
  int N1, N2, N3;
  ...
  for (int i = 0; i < static_cast<int>(this->getNoElms(false)); ++i) {
    ...
    if (i % (N1*N2) < N1*(N2-1))
      neigh[idx][3] = idx+N1;
    if (i / (N1*N2) > 0)
      neigh[idx][4] = idx - (N1*N2);
    if (i / (N1*N2) < N3-1)
      neigh[idx][5] = idx + (N1*N2);
  }
}
...
void ASMs3D::getBoundaryElms (int lIndex, int, IntVec& elms) const
{
  ...
  if (lIndex == 1 || lIndex == 2) {
    ...
    for (int i = 0; i < N3m; ++i)
      for (int j = 0; j < N2m; ++j)
        elms.push_back(this->getElmID(j*N1m + i*N1m*N2m + (lIndex-1)*(N1m-1)+1)-1);
  } else if (lIndex == 3 || lIndex == 4) {
    elms.reserve(N1m*N3m);
    for (int i = 0; i < N3m; ++i)
      for (int j = 0; j < N1m; ++j)
        elms.push_back(this->getElmID(j + i*N1m*N2m + (lIndex-3) * (N1m*(N2m-1))+1)-1);
  } else {
