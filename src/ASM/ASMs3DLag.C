                           ...
                           double* xi, double* eta, double* zeta) const
{
  ...
  const std::array<int,3> elm {
    std::get<1>(knot[0]) - (p1 - 1),
    std::get<1>(knot[1]) - (p2 - 1),
    std::get<1>(knot[2]) - (p3 - 1)
  };
  ...
  auto getParam = [this](int dir, const auto& p)
  {
    const double knot_1 = *(svol->basis(dir).begin() + std::get<1>(p[dir]));
    const double knot_2 = *(svol->basis(dir).begin() + std::get<1>(p[dir]) + 1);
    return -1.0 + (std::get<0>(p[dir]) - knot_1) / (knot_2 - knot_1) * 2.0;
  };
