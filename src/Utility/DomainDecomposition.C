...
void DomainDecomposition::calcGroups (size_t nel1, size_t g1, size_t overlap)
{
  ...
  else
  {
    ...
    for (size_t gu = 0; gu < g1; ++gu) {
      ...
      if (gu == 0 || gu == g1-1)
        o1 -= overlap;
      subdomains[gu].reserve(nel1_sub+o1);
      for (size_t i = 0; i < nel1_sub+o1 && ofs1 + i < nel1; ++i)
        subdomains[gu].push_back(ofs1 + i);
      ofs1 += nel1_sub + o1 - overlap;
    }
  }
}
  ...
  else
  {
    ...
    for (size_t gv = 0; gv < g2; ++gv) {
      ...
      for (size_t gu = 0; gu < g1; ++gu, ++g) {
        ...
        if (gu == 0 || gu == g1-1)
          o1 -= overlap;
        subdomains[g].reserve((nel1_sub+o1)*(nel2_sub+o2));
        for (size_t j = 0; j < nel2_sub+o2 && ofs2 + j < nel2; ++j)
          for (size_t i = 0; i < nel1_sub+o1 && ofs1 + i < nel1; ++i)
            subdomains[g].push_back((ofs2+j)*nel1 + ofs1 + i);
        ofs1 += nel1_sub+o1 - overlap;
      }
      ofs2 += nel2_sub + o2 - overlap;
    }
  }
}
    ...
    for (size_t gw = 0; gw < g3; ++gw) {
      ...
      for (size_t gv = 0; gv < g2; ++gv) {
        ...
        for (size_t gu = 0; gu < g1; ++gu, ++g) {
          ...
          if (gu == 0 || gu == g1-1)
            o1 -= overlap;
          subdomains[g].reserve((nel1_sub+o1)*(nel2_sub+o2)*(nel3_sub+o3));
          for (size_t k = 0; k < nel3_sub+o3 && ofs3 + k < nel3; ++k)
            for (size_t j = 0; j < nel2_sub+o2 && ofs2 + j < nel2; ++j)
              for (size_t i = 0; i < nel1_sub+o1 && ofs1 + i < nel1; ++i)
