  ...
  : gradient(nullptr), dx(eps)
{
  try {
    ...
    for (size_t i = 0; i < nalloc; ++i) {
      expr[i] = new ExprEval::Expression<Real>;
      f[i] = new ExprEval::FunctionList<Real>;
      v[i] = new ExprEval::ValueList<Real>;
...
{
  for (ExprEval::Expression<Real>* it : expr)
    delete it;
  for (ExprEval::FunctionList<Real>* it : f)
    delete it;
  for (ExprEval::ValueList<Real>* it : v)
    delete it;
  delete gradient;
  expr.clear();
  f.clear();
...
{
  for (ExprEval::ValueList<Real>* v1 : v) {
