...
TEST(TestAnaSol, VectorAD)
{
  ...
  AnaSol mySol(tag,true);
  VecFunc* v = mySol.getVectorSol();
  ASSERT_TRUE(v != nullptr);

  Vec3 X(1.0,2.0,3.0);
  const Vec3 val = (*v)(X);
  EXPECT_FLOAT_EQ(val[0],8.0);
  EXPECT_FLOAT_EQ(val[1],4.0);
  EXPECT_FLOAT_EQ(val[2],3.0);
}
