...
bool utl::parseKnots (std::vector<Real>& xi)
{
  ...
  else if (isalpha(cline[0]))
  {
    ...
    bool centered = toupper(cline[0]) == 'C';
    ...
    if (centered) {
      double x = xi1;
      double C = (xi2 - xi1) / 2.0;
      C *= (alpha == 1.0 ? 1.0/Real(ru/2+1) : (1.0-alpha)/(1.0-pow(alpha,1+ru/2)));
      if (xi1 > 0.0) xi.push_back(xi1);
      for (int i = 0; i < ru / 2; i++)
      {
        x += C;
        C *= alpha;
        xi.push_back(x);
      }
      xi.push_back(xi1 + (xi2-xi1)/2.0);
      for (int i = 0; i < ru / 2; ++i)
        xi.push_back(xi2 - xi[ru/2-1-i]+xi1);
