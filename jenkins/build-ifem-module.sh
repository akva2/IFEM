...
function clone_module {
  ...
  GH_USER[IFEM-PoroElasticity]=akva2
...
function build_module_and_upstreams {
  ...
  do
    ...
    pushd .
    mkdir ${BTYPES_ARRAY[$BTYPE]}/build-$1
    cd ${BTYPES_ARRAY[$BTYPE]}/build-$1
    build_module "-DCMAKE_INSTALL_PREFIX=$WORKSPACE/${BTYPES_ARRAY[$BTYPE]}/install -DCMAKE_PREFIX_PATH=$WORKSPACE/${BTYPES_ARRAY[$BTYPE]}/install -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE}" 1 $WORKSPACE
    test $? -eq 0 || exit 1
    popd
