    ...
    foreach(source ${ARGN})
        ...
        foreach(hit ${found_tests2})
          ...
          foreach(ghit ${found_tests3})
            ...
            math(EXPR UNIT_TEST_NUMBER "${UNIT_TEST_NUMBER}+1")
            set(UNIT_TEST${UNIT_TEST_NUMBER} ${test_prefix}.${ghit_tname} ${working_dir} "${executable}" --gtest_filter=${filter_name}.${ghit_tname}* CACHE STRING "" FORCE)
          endforeach()
        endforeach()
    endforeach()
...
macro(IFEM_add_unittests IFEM_PATH)
  ...
  # Parallel unit tests. They all run with exactly 4 processes.
  if(MPI_FOUND)
    ...
    gtest_add_tests("${MPIEXEC} -np 4 $<TARGET_FILE:IFEM-MPI-test>" ${IFEM_PATH} ${TEST_SRCS})
...
macro(add_check_target)
  ...
  if (${UNIT_TEST_NUMBER} GREATER 0)
    foreach(test_number RANGE 1 ${UNIT_TEST_NUMBER})
      ...
      list(REMOVE_AT UNIT_TEST${test_number} 1)
      list(REMOVE_AT UNIT_TEST${test_number} 0)
      add_test(NAME ${name} WORKING_DIRECTORY ${dir} COMMAND ${UNIT_TEST${test_number}})
    endforeach()
  endif()
